...
export default function ProfesionalPage({ user }) {
    ...
    const handleSelect = (professional) => {
        navigate('/calendario', {
            state: { service, professional, bookingToChange: location.state?.bookingToChange }
        })
    }

    const handleIndifferent = () => {
        navigate('/calendario', {
            state: { service, professional: null, bookingToChange: location.state?.bookingToChange }
        })
    }
    ...
    const getProfessionalIcon = (pro) => {
        ...
        const photoSrc = getPhotoSrc(pro)

        if (photoSrc) {
            return (
                <div style={photoStyle}>
                    <img src={photoSrc} alt={pro.name} style={imgStyle} />
                </div>
            )
        }
                        ...
                        >
                            <div style={{ display: 'flex', marginRight: '20px' }}>
                                {professionals.slice(0, 3).map((pro, idx) => {
                                    const src = getPhotoSrc(pro)
                                    return (
                                        <div key={pro.id} style={{ width: '50px', height: '50px', borderRadius: '50%', overflow: 'hidden', border: '2px solid var(--accent)', marginRight: idx < 2 ? '-15px' : '0', zIndex: 3 - idx, background: '#000' }}>
                                            {src ? (
                                                <img src={src} alt={pro.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                            ) : (
                                                <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="1.2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>
                                            )}
                                        </div>
                                    )
                                })}
